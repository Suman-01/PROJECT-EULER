...
for i in range(20):
    for j in range(n_rows - k + 1):
        max_prod.append(matrix[i][j] * matrix[i][j + 1] * matrix[i][j + 2] * matrix[i][j + 3])
        max_prod.append(matrix[j][i] * matrix[j + 1][i] * matrix[j + 2][i] * matrix[j + 3][i])

    
for i in range(n_cols - k + 1):
    for j in range(n_rows - k + 1):
        max_prod.append(matrix[i][j] * matrix[i + 1][j + 1] * matrix[i + 2][j + 2] * matrix[i + 3][j + 3])
